...
def bilateral_blur_3x3(
    ...
    sigma_range = max(float(sigma_range), 1e-6)
    spatial_kernel = np.array(
        [
            [math.exp(-1.0), math.exp(-0.5), math.exp(-1.0)],
            [math.exp(-0.5), 1.0, math.exp(-0.5)],
            [math.exp(-1.0), math.exp(-0.5), math.exp(-1.0)],
        ],
        dtype=np.float32,
    )
    ...
    for _ in range(passes):
        src_pad = np.pad(out, 1, mode="edge")
        guide_pad = np.pad(guide, 1, mode="edge")
        weight_pad = np.pad(weight, 1, mode="constant", constant_values=0.0)
        ...
        for oy in range(3):
            for ox in range(3):
                neighbor_src = src_pad[oy:oy + h, ox:ox + w]
                neighbor_guide = guide_pad[oy:oy + h, ox:ox + w]
                neighbor_weight = weight_pad[oy:oy + h, ox:ox + w]
                range_term = np.exp(-0.5 * np.square((neighbor_guide - center_guide) / sigma_range))
                kernel = spatial_kernel[oy, ox] * neighbor_weight * range_term
                accum += kernel * neighbor_src
                norm += kernel

        next_out = out.copy()
        np.divide(accum, norm, out=next_out, where=norm > 1e-6)
        out = next_out
        out = np.where(weight > 1e-6, out, 0.0)
...
def gaussian_splat_render(
    ...
    coverage_softness: float,
    depth_bilateral_sigma_scale: float,
    normal_source: str,
    ...
    depth_weight = np.clip(tau, 0.0, 1.0)
    depth_sigma = max(depth_bilateral_sigma_scale * particle_radius, 1e-6)
    smooth_depth = bilateral_blur_3x3(
        src=mean_depth,
        guide=mean_depth,
        weight=depth_weight,
        passes=thickness_blur_passes,
        sigma_range=depth_sigma,
    )
    ...
    coverage = smoothstep(coverage_threshold, coverage_threshold + coverage_softness, tau)
    opacity = (1.0 - np.exp(-tau * opacity_gain)) * coverage
...
def main():
    ...
    parser.add_argument("--time_step_override", type=float, default=0.0, help="If >0, overrides scene timeStepSize")
    ...
    parser.add_argument("--sigma_scale", type=float, default=0.85, help="Controls Gaussian footprint size (small-kernel preset)")
    parser.add_argument("--alpha_scale", type=float, default=0.90, help="Controls thickness contribution per splat")
    parser.add_argument("--min_sigma", type=float, default=0.35)
    parser.add_argument("--max_sigma", type=float, default=2.8)
    parser.add_argument("--max_kernel_radius", type=int, default=9, help="Clamp per-particle kernel radius in pixels")
    ...
    parser.add_argument("--thickness_blur_passes", type=int, default=1, help="Depth-aware smoothing passes before normal extraction")
    parser.add_argument("--coverage_threshold", type=float, default=0.10, help="Suppresses low-thickness halo before final opacity")
    parser.add_argument("--coverage_softness", type=float, default=0.18, help="Smooth transition width for silhouette coverage")
    parser.add_argument(
        "--depth_bilateral_sigma_scale",
        type=float,
        default=3.0,
        help="Depth bilateral range sigma in particle-radius units",
    )
    parser.add_argument(
        "--normal_source",
        default="hybrid",
    ...
    if args.time_step_override > 0.0:
        config.config["Configuration"]["timeStepSize"] = float(args.time_step_override)
    ...
    for frame_idx in range(args.frames):
        ...
        frame = gaussian_splat_render(
            ...
            coverage_softness=args.coverage_softness,
            depth_bilateral_sigma_scale=args.depth_bilateral_sigma_scale,
