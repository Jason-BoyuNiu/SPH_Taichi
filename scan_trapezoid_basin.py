...
def run():
    ...
    parser.add_argument("--background_mode", default="checker", choices=["plain", "studio", "checker"])
    parser.add_argument("--sigma_scale", type=float, default=1.6)
    parser.add_argument("--alpha_scale", type=float, default=0.75)
    parser.add_argument("--ior", type=float, default=1.333)
    parser.add_argument("--ior_dispersion", type=float, default=0.004)
    parser.add_argument("--opacity_gain", type=float, default=0.8)
    parser.add_argument("--kinetic_percentile", type=float, default=-1.0)
    parser.add_argument("--specular_strength", type=float, default=0.85)
    parser.add_argument("--fresnel_f0", type=float, default=0.02)
    parser.add_argument("--sun_power", type=float, default=384.0)
    parser.add_argument("--include_solids", action="store_true", help="Render the container (solid boundaries) along with fluid")
    ...
    for i, (L, tw, v) in enumerate(cases, start=1):
        ...
        cmd = [
            ...
            "60",
            "--sigma_scale",
            str(args.sigma_scale),
            "--alpha_scale",
            str(args.alpha_scale),
            "--ior",
            str(args.ior),
            "--ior_dispersion",
            str(args.ior_dispersion),
            "--opacity_gain",
            str(args.opacity_gain),
            "--specular_strength",
            str(args.specular_strength),
            "--fresnel_f0",
            str(args.fresnel_f0),
            "--sun_power",
            str(args.sun_power),
            ...
            "--kinetic_percentile",
            str(args.kinetic_percentile),
            "--substeps_override",
            str(args.substeps_override),
        ]

        if args.include_solids:
            cmd.append("--include_solids")
